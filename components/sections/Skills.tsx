...
const skillCategories: SkillCategory[] = [
  ...
  },
  {
    ...
    icon: Wrench,
    skills: ['TensorFlow', 'PyTorch', 'Scikit-Learn', 'React.js', 'Node.js','Django','Beautiful Soup'],
    color: 'teal'
  },
  {
    title: 'Tools & Platforms',
    icon: Cloud,
    skills: ['Git', 'Docker', 'AWS','Microsoft Azure', 'Jupyter Notebooks', 'VS Code', ],
    color: 'purple'
  },
  {
    title: 'Data & Analytics',
    icon: Database,
    skills: ['NumPy', 'Pandas','Seaborn', 'MySQL','Microsoft Excel','Power BI', 'Tableau ','MongoDB','cassandra' ],
    color: 'orange'
  }
];
...
const badgeColorVariants: Record<SkillCategoryColor, string> = {
  ...
  purple: 'bg-purple-50/50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-300',
  orange: 'bg-orange-50/50 text-orange-700 dark:bg-orange-900/20 dark:text-orange-300'
};
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <Card className={`h-full border-2 transition-all duration-300 hover:shadow-lg bg-background/50 backdrop-blur-sm ${colorVariants[category.color]}`}>
                <CardHeader>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {category.skills.map((skill, skillIndex) => (
                      ...
                      >
                        <Badge 
                          variant="secondary"
                          className={`${badgeColorVariants[category.color]} hover:shadow-md transition-all duration-200 cursor-default backdrop-blur-sm`}
                        >
                          {skill}
                        </Badge>
                      </motion.div>
